...
int tc;
int steps;
float expecteddiamondvalue(int start,int end,int level) {
...
/*		if ( (end - start) == 2 ) {
			cout<<2*diamonds[start] + diamonds[start+1] + diamonds[end] + 2*diamonds[end] + diamonds[start] + diamonds[start + 1]<<endl;
				used[start] = used[start] + 3;
				used[start+1] = used[start+1] + 2;
				used[end] = used[end] + 3;
				steps = steps+2;
			return (2*diamonds[start] + diamonds[start+1] + diamonds[end] + 2*diamonds[end] + diamonds[start] + diamonds[start + 1]);
		}*/
		if (start == end) {
			cout<<diamonds[start];
				used[start] = used[start] + 1;	
				cout<<"Steps 1 :"<<steps<<endl;
				cout<<"Level I:"<<level<<endl;
			steps = steps+1;
				cout<<"Steps 1 :A"<<steps<<endl;
		...
		cout<< diamonds[start] + diamonds[end]<<endl ;
				used[start] = used[start] + 1;
				used[end] = used[end] + 1;
				cout<<"Steps 2 :"<<steps<<endl;
				cout<<"Level I:"<<level<<endl;
				steps = steps+2;
				cout<<"Steps 2 :A"<<steps<<endl;
			return (diamonds[start] + diamonds[end] );
		}
...
//		return	max2[start][end];
	...
	level = level-1;
		max2[start+2][end] = expecteddiamondvalue(start+2,end,level-1);
//used[start+2][end] = tc+1;

	//	cout<<"Start+2"<<"end"<<endl;
	max2[start+1][end-1] = expecteddiamondvalue(start+1,end-1,level-1);
	max2[start+1][end-1] = expecteddiamondvalue(start+1,end-1,level-1);
//	used[start+1][end-1] = tc+1;

		max2[start][end-2] = expecteddiamondvalue(start,end-2,level-1);
//		used[start][end-2] = tc+1;
	cout<<"LEVELE "<<level+1;
	cout<<"snkj"<<(pow(2,level+1)*diamonds[start] + pow(2,level+1)*diamonds[end] + max2[start+2][end] + 2*max2[start+1][end-1] + max2[start][end-2])<<endl;
				used[start] = used[start] + 2;
				used[end] = used[end] + 2;
	return (float)(pow(2,level+1)*diamonds[start] + pow(2,level+1)*diamonds[end] + max2[start+2][end] + 2*max2[start+1][end-1] + max2[start][end-2]);
