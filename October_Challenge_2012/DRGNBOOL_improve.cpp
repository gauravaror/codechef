...
int main() {
		...
		for (uint32_t i = 0; i < testcases; ++i) {
				...
				for ( uint32_t j  = 0;j < sointN ; j++ ) {
					...
					if (solevelscore.find(level) == solevelscore.end()) {
						solevelscore[level] = chakra;
					} else {
						solevelscore[level] = solevelscore.find(level)->second + chakra;
					}
				}
	...
	//To check how much time actual logic takes
				...
				for ( chakrainterator = solevelscore.begin();chakrainterator != solevelscore.end();chakrainterator++) {
					if ( (*chakrainterator).second < 0 ) {
						minimumchakra += (*chakrainterator).second; 						
					}
				}
				minimumchakra = -1 * minimumchakra;
