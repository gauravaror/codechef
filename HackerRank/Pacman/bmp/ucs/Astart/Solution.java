...
//                    System.out.println("DEBUG("+(pacman_r)+","+(pacman_c)+")"+""+"ToArray: "+distance);
        ...
        if (pacman_r-1 > 0 && pacman_c  <c) {
            int i = pacman_r-1;
            int j = pacman_c;    
            executelogic(i,j,pacman_r,pacman_c,distance,grid,food_r,food_c);
        }
...
private static void executelogic(int i,int j,int pacman_r,int pacman_c, int distance,String [][] grid,int food_r,int food_c) {
            if ((grid[i][j].equals("-") || grid[i][j].equals(".") )  && nodedetails[i][j].getDistance() == -100)  {
                lifonode li = nodedetails[i][j];
                 int h =    Math.abs(food_r-pacman_r) + Math.abs(food_c-pacman_r);
                li.setDistance(distance+ h);
                li.setPX(pacman_r);
                li.setPY(pacman_c);
